...
class CustomNavigationBarCubit extends Cubit<CustomNavigationBarState> {
  CustomNavigationBarCubit({required this.router})
    : super(const CustomNavigationBarState());
  ...
  FutureOr<void> onLocationChanged({required String location}) {
    if (router.isMainRoute(location) &&
        location != pageToLocation(state.currentPage.$1)) {
